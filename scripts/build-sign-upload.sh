...
# Requires: source ~/.secrets/termmesh.env && export SPARKLE_PRIVATE_KEY
...
ENTITLEMENTS="term-mesh.entitlements"
APP_PATH="build/Build/Products/Release/term-mesh.app"

# --- Pre-flight ---
source ~/.secrets/termmesh.env
...
echo "Building app..."
rm -rf build/
xcodebuild -scheme term-mesh -configuration Release -derivedDataPath build CODE_SIGNING_ALLOWED=NO build 2>&1 | tail -5
...
/usr/libexec/PlistBuddy -c "Add :SUFeedURL string https://github.com/manaflow-ai/term-mesh/releases/latest/download/appcast.xml" "$APP_PLIST"
echo "Sparkle keys injected"

# --- Codesign ---
echo "Codesigning..."
CLI_PATH="$APP_PATH/Contents/Resources/bin/term-mesh"
if [ -f "$CLI_PATH" ]; then
...
# --- Notarize app ---
echo "Notarizing app..."
ditto -c -k --sequesterRsrc --keepParent "$APP_PATH" term-mesh-notary.zip
xcrun notarytool submit term-mesh-notary.zip \
  --apple-id "$APPLE_ID" --team-id "$APPLE_TEAM_ID" --password "$APPLE_APP_SPECIFIC_PASSWORD" --wait
xcrun stapler staple "$APP_PATH"
xcrun stapler validate "$APP_PATH"
rm -f term-mesh-notary.zip
echo "App notarized"
...
cask "term-mesh" do
  ...
  url "https://github.com/manaflow-ai/term-mesh/releases/download/v#{version}/term-mesh-macos.dmg"
  name "term-mesh"
  desc "Lightweight native macOS terminal with vertical tabs for AI coding agents"
  homepage "https://github.com/manaflow-ai/term-mesh"
  ...
  zap trash: [
    ...
    "~/Library/Caches/term-mesh",
    "~/Library/Preferences/com.termmesh.app.plist",
  ]
end
CASKEOF
    cd homebrew-cmux
    git add Casks/term-mesh.rb
    if git diff --staged --quiet; then
      echo "Homebrew cask already up to date"
    else
      git commit -m "Update term-mesh to ${VERSION}"
      git push
