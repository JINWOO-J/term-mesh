...
def focused_pane_id(client: termmesh) -> Optional[str]:
...
def test_cmd_bracket_back_forward(client: termmesh) -> tuple[bool, str]:
    """
    ...
    client.navigate(browser_id, "https://example.org")
    time.sleep(2.0)
    ...
    url_after_back = client.get_url(browser_id)
    ...
    url_after_forward = client.get_url(browser_id)
...
def test_cmd_bracket_noop_on_terminal(client: termmesh) -> tuple[bool, str]:
    """
...
def test_browser_back_forward_socket_commands(client: termmesh) -> tuple[bool, str]:
    """
    ...
    # browser_back
    try:
        client.browser_back(browser_id)
    except termmeshError as e:
        client.close_workspace(ws_id)
        return False, f"browser_back command failed: {e}"
    time.sleep(1.5)
    ...
    # browser_forward
    try:
        client.browser_forward(browser_id)
    except termmeshError as e:
        client.close_workspace(ws_id)
        return False, f"browser_forward command failed: {e}"
    time.sleep(1.5)
