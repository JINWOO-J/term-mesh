...
def _verify_responsive(c: termmesh, marker: Path, surface_idx: int, retries: int = 3) -> bool:
...
def _wait_terminal_in_window(c: termmesh, surface_idx: int, timeout: float = 5.0) -> bool:
    start = time.time()
    while time.time() - start < timeout:
        try:
...
def test_multi_workspace_terminal_responsive(c: termmesh) -> None:
    """
...
def test_three_workspaces_non_last_responsive(c: termmesh) -> None:
    """
...
def test_rapid_workspace_switching_preserves_focus(c: termmesh) -> None:
    """
...
def test_browser_panel_focus_and_return(c: termmesh) -> None:
    """
    ...
    if not surfaces:
        raise termmeshError("No surfaces after new_workspace")
...
def test_browser_focus_across_workspaces(c: termmesh) -> None:
    """
    ...
    with termmesh() as c:
        ...
        for name, test_fn in tests:
            ...
            try:
                ...
                print("PASS")
                passed += 1
            except (AssertionError, termmeshError) as e:
                print(f"FAIL: {e}")
                failed += 1
