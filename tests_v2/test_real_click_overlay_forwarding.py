...
Usage:
    python3 tests_v2/test_real_click_overlay_forwarding.py
"""
...
def wait_for_terminal_focus(client: termmesh, panel_id: str, timeout_s: float = 2.0) -> bool:
    start = time.time()
...
def main() -> int:
    ...
    with termmesh(socket_path) as client:
        ...
        try:
            ...
            client._call("debug.drag_pasteboard.seed", {"type": "fileurl"})
            client.activate_app()
            time.sleep(0.2)
            ...
            try:
                client._call("debug.drag_pasteboard.clear")
            except Exception:
                pass
            if ws_id:
                try:
