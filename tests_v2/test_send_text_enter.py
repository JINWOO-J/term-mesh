...
Usage:
    python3 tests_v2/test_send_text_enter.py
...
def test_send_text_with_newline(client: termmesh) -> TestResult:
    ...
    try:
        ...
        if _wait_marker(marker):
            ...
            screen = client.read_terminal_text()
            result.failure(
...
def test_read_screen_after_send(client: termmesh) -> TestResult:
    """
    Verify that command output appears in read_terminal_text after send+Enter.
    This is a higher-level check that the command actually ran.
    """
    result = TestResult("read_terminal_text() shows command output after send")
