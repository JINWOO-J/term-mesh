...
Run with a tagged instance to avoid unix socket conflicts:
  TERMMESH_TAG=<tag> python3 tests_v2/test_sidebar_ports.py
"""
...
def _start_external_server(base: Path, port: int) -> subprocess.Popen:
    """
    Start an http.server outside term-mesh and ensure it is actually listening.
...
def _wait_for_port(client: termmesh, port: int, timeout: float = 18.0) -> dict[str, str]:
    def pred():
...
def _wait_for_port_absent(client: termmesh, port: int, timeout: float = 18.0) -> dict[str, str]:
    def pred():
        state = _parse_sidebar_state(client.sidebar_state())
        raw = state.get("ports", "")
        if raw == "none" or not raw:
...
def _assert_port_absent_for_duration(client: termmesh, port: int, duration: float = 6.0, interval: float = 0.15) -> None:
    """
...
def main() -> int:
    ...
    base = Path("/tmp") / f"termmesh_ports_test_{os.getpid()}"
    ...
    try:
        ...
        # Start a listening server outside term-mesh. A fresh tab should NOT show this port,
        ...
        with termmesh() as client:
            ...
            client.send("echo termmesh_ports_test\n")
    ...
    except (termmeshError, AssertionError, RuntimeError, ValueError) as e:
