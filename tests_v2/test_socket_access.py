...
Usage:
    python3 tests_v2/test_socket_access.py
"""
...
def _derived_app_candidates_for_current_worktree():
    ...
    for info_path in info_paths:
        ...
        app_path = os.path.join(derived_root, "Build/Products/Debug/term-mesh DEV.app")
...
def _find_app():
    ...
    if preferred_slug:
        preferred_tmp = []
        preferred_tmp.extend(glob.glob(f"/tmp/termmesh-{preferred_slug}/Build/Products/Debug/term-mesh DEV*.app"))
        preferred_tmp.extend(glob.glob(f"/private/tmp/termmesh-{preferred_slug}/Build/Products/Debug/term-mesh DEV*.app"))
    ...
    derived_candidates = glob.glob(os.path.join(
        home, "Library/Developer/Xcode/DerivedData/*/Build/Products/Debug/term-mesh DEV.app"
    ))
    tmp_candidates = []
    tmp_candidates.extend(glob.glob("/tmp/termmesh-*/Build/Products/Debug/term-mesh DEV*.app"))
    tmp_candidates.extend(glob.glob("/private/tmp/termmesh-*/Build/Products/Debug/term-mesh DEV*.app"))
...
def _find_cli(preferred_app_path: str = ""):
    ...
    if preferred_app_path:
        ...
        sibling = os.path.join(debug_dir, "term-mesh")
    ...
    candidates.extend(glob.glob(os.path.join(
        home, "Library/Developer/Xcode/DerivedData/*/Build/Products/Debug/term-mesh"
    )))
    candidates.extend(glob.glob("/tmp/termmesh-*/Build/Products/Debug/term-mesh"))
    candidates.extend(glob.glob("/private/tmp/termmesh-*/Build/Products/Debug/term-mesh"))
...
def _kill_app(app_path: str = None):
    if app_path:
        exe = os.path.join(app_path, "Contents/MacOS/term-mesh DEV")
        subprocess.run(["pkill", "-f", exe], capture_output=True)
    else:
        subprocess.run(["pkill", "-x", "term-mesh DEV"], capture_output=True)
    time.sleep(1.5)


def _launch_app(app_path: str, socket_path: str, mode: str = None, extra_env: dict = None):
    if os.path.exists(socket_path):
        try:
...
def test_allowall_mode_works(socket_path: str, app_path: str) -> TestResult:
    """Verify TERMMESH_SOCKET_MODE=allowAll bypasses ancestry check."""
    result = TestResult("allowAll mode allows external")
    try:
        _kill_app(app_path)
        _launch_app(app_path, socket_path, mode="allowAll")
...
def test_password_mode_requires_auth(socket_path: str, app_path: str) -> TestResult:
    ...
    password = f"termmesh-pass-{os.getpid()}"
    try:
        _kill_app(app_path)
        _launch_app(
            app_path,
...
def test_password_mode_cli_exit_code(socket_path: str, app_path: str) -> TestResult:
    ...
    try:
        ...
        if not cli_path:
            result.failure("Could not find term-mesh CLI binary")
            return result
...
def run_tests():
    print("=" * 60)
    print("term-mesh Socket Access Control Tests")
    print("=" * 60)
    print()
